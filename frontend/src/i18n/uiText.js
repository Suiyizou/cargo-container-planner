const uiText = {
  ...
  "algorithm.step.recommendText": {
    zh: "每个箱型分别试算。推荐顺序综合合规状态、参考费用档、箱数、利用率区间和特种箱型惩罚，不再单纯追求最高空间占用率；如果前一个箱型已经单箱成功，后续更大箱型会先检查能否复用同一套坐标，减少重复计算。",
    en: "Each container type is evaluated separately. Ranking combines compliance, reference cost tier, box count, utilization band, and special-equipment penalties instead of chasing only the highest fill rate. If an earlier container succeeds in one box, larger containers first check whether the same coordinates can be reused."
  },
